...
public class WageDistributionController {
    ...
    WageDistributionController(WageDistributionEntityRepository wageDistributionEntityRepository, SubCategoryEntityRepository subCategoryEntityRepository, CategoryEntityRepository categoryEntityRepository) {
    ...
    EntityModel<SubCategory> getSubCategory(@PathVariable String categoryName, @PathVariable String subCategoryName, boolean... selfRelOnly) {
        ...
        if (selfRelOnly != null && selfRelOnly[0] == true)
            return EntityModel.of(subCategoryResource,
                linkTo(methodOn(WageDistributionController.class).getSubCategory(categoryName, subCategoryName)).withSelfRel(),
    ...
    EntityModel<Year> getYear(@PathVariable String categoryName, @PathVariable String subCategoryName, @PathVariable int year, boolean... selfRelOnly) {
        List<WageDistributionEntity> wageDistributionEntities = wageDistributionEntityRepository.findWageDistributionBySubCategoryAndYear(getSubCategoryEntity(categoryName, subCategoryName), year);
        if (wageDistributionEntities.isEmpty()) throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        List<EntityModel<WageGroup>> wageDistributionEntityModels = wageDistributionEntities
            .stream()
            .map(wageDistributionEntity -> getWageGroup(categoryName, subCategoryName, year, wageDistributionEntity.getWageGroup().getName(), true))
            .collect(Collectors.toList());
        Year yearResource = new Year(year, CollectionModel.of(wageDistributionEntityModels));
        if (selfRelOnly != null && selfRelOnly[0] == true)
            return EntityModel.of(yearResource,
                linkTo(methodOn(WageDistributionController.class).getYear(categoryName, subCategoryName, year)).withSelfRel());
        return EntityModel.of(yearResource,
            linkTo(methodOn(WageDistributionController.class).getYear(categoryName, subCategoryName, year)).withSelfRel(),
            linkTo(methodOn(WageDistributionController.class).getSubCategory(categoryName, subCategoryName)).withRel("subCategory"));
    }

    @GetMapping("/{categoryName}/{subCategoryName}/{year}/{wageGroup}")
    EntityModel<WageGroup> getWageGroup(@PathVariable String categoryName, @PathVariable String subCategoryName, @PathVariable int year, @PathVariable String wageGroup, boolean... selfRelOnly) {
        WageDistributionEntity wageDistributionEntity = wageDistributionEntityRepository.findWageDistributionBySubCategoryAndYearAndWageGroup(
            getSubCategoryEntity(categoryName, subCategoryName), year, getWageGroupEntity(wageGroup));
        if (wageDistributionEntity == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND);
    ...
    private SubCategoryEntity getWageGroupEntity(String wageGroup) {
        SubCategoryEntity wageGroupEntity = subCategoryEntityRepository.findSubCategoryByCategoryNameAndName("wageGroup", wageGroup);
        if (wageGroupEntity == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND);
