...
public class EmployeeCountController {
	...
	EntityModel<SubCategory> getSubCategory(@PathVariable String categoryName, @PathVariable String subCategoryName,
			boolean... selfRelOnly) {
		List<EmployeeCountEntity> employeeCountEntities = employeeCountEntityRepository
				.findEmployeeCountBySubCategory(getSubCategoryEntity(categoryName, subCategoryName));
		// if (employeeCountEntities.isEmpty())
		// throw new ResponseStatusException(HttpStatus.NOT_FOUND);
		List<EntityModel<Year>> yearEntityModels = employeeCountEntities.stream().distinct()
		...
		if (selfRelOnly != null && selfRelOnly[0] == true)
			return EntityModel.of(subCategoryResource,
					linkTo(methodOn(EmployeeCountController.class).getSubCategory(categoryName, subCategoryName))
							.withSelfRel(),
	...
	EntityModel<Year> getYear(@PathVariable String categoryName, @PathVariable String subCategoryName,
		...
		List<EmployeeCountEntity> employeeCountEntities = employeeCountEntityRepository
				.findEmployeeCountBySubCategoryAndYear(getSubCategoryEntity(categoryName, subCategoryName), year);
		if (employeeCountEntities.isEmpty())
			throw new ResponseStatusException(HttpStatus.NOT_FOUND);
		List<EntityModel<Gender>> genderEntityModels = employeeCountEntities.stream()
				.map(employeeCountEntity -> getGender(categoryName, subCategoryName, year,
						Gender.toString(employeeCountEntity.getSex()), true))
				.collect(Collectors.toList());
		Year yearResource = new Year(year, CollectionModel.of(genderEntityModels));
		if (selfRelOnly != null && selfRelOnly[0] == true)
			return EntityModel.of(yearResource,
					linkTo(methodOn(EmployeeCountController.class).getYear(categoryName, subCategoryName, year))
							.withSelfRel());
		return EntityModel.of(yearResource,
				linkTo(methodOn(EmployeeCountController.class).getYear(categoryName, subCategoryName, year))
						.withSelfRel(),
	...
	EntityModel<Gender> getGender(@PathVariable String categoryName, @PathVariable String subCategoryName,
		...
		EmployeeCountEntity employeeCountEntity = employeeCountEntityRepository
				.findEmployeeCountBySubCategoryAndYearAndSex(getSubCategoryEntity(categoryName, subCategoryName), year,
		...
		if (selfRelOnly != null && selfRelOnly[0] == true)
			return EntityModel.of(genderResource, linkTo(
					methodOn(EmployeeCountController.class).getGender(categoryName, subCategoryName, year, gender))
							.withSelfRel());
		return EntityModel.of(genderResource,
				linkTo(methodOn(EmployeeCountController.class).getGender(categoryName, subCategoryName, year, gender))
						.withSelfRel(),
				linkTo(methodOn(EmployeeCountController.class).getYear(categoryName, subCategoryName, year))
						.withRel("year"));
	}
