...
public class AverageWageController {
    ...
    AverageWageController(AverageWageEntityRepository averageWageEntityRepository,
    ...
    EntityModel<SubCategory> getSubCategory(@PathVariable String categoryName, @PathVariable String subCategoryName,
            boolean... selfRelOnly) {
        List<AverageWageEntity> averageWageEntities = averageWageEntityRepository
                .findAverageWageBySubCategory(getSubCategoryEntity(categoryName, subCategoryName));
        if (averageWageEntities.isEmpty())
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        List<EntityModel<Year>> yearEntityModels = averageWageEntities.stream().distinct()
        ...
        if (selfRelOnly != null && selfRelOnly[0] == true)
            return EntityModel.of(subCategoryResource,
                    linkTo(methodOn(AverageWageController.class).getSubCategory(categoryName, subCategoryName))
                            .withSelfRel(),
    ...
    EntityModel<Year> getYear(@PathVariable String categoryName, @PathVariable String subCategoryName,
        ...
        List<AverageWageEntity> averageWageEntities = averageWageEntityRepository
                .findAverageWageBySubCategoryAndYear(getSubCategoryEntity(categoryName, subCategoryName), year);
        if (averageWageEntities.isEmpty())
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        List<EntityModel<Gender>> genderEntityModels = averageWageEntities.stream()
                .map(averageWageEntity -> getGender(categoryName, subCategoryName, year,
                        Gender.toString(averageWageEntity.getSex()), true))
                .collect(Collectors.toList());
